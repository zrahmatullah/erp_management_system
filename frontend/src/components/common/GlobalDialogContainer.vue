  ...
  <Teleport to="body">
    <Transition name="dialog-fade">
      <div
        ...
        class="fixed md:left-64 inset-y-0 right-0 left-0 z-9999 flex items-center justify-center bg-slate-950/45 backdrop-blur-sm p-4 transition-all duration-200"
        @click.self="dialogStore.handleCancel"
        @keydown.esc="dialogStore.handleCancel"
      >
      <div
        class="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl shadow-slate-950/25 border border-slate-100/90 transform transition-all duration-300 ease-out"
        role="dialog"
        aria-modal="true"
      >
        <div class="flex items-start gap-4">
          <!-- Icon Badge -->
          <div
            class="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 border shadow-xs"
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between">
              ...
              <button
                ...
                class="text-slate-400 hover:text-slate-600 p-1.5 rounded-xl hover:bg-slate-100 transition-colors -mt-1 -mr-1 cursor-pointer"
              >
                <X class="w-4 h-4" />
              </button>
            </div>
            ...
            <form v-if="dialogStore.options.type === 'prompt'" @submit.prevent="dialogStore.handleConfirm" class="mt-4">
              <input
                ...
                class="w-full px-3.5 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-hidden font-semibold text-slate-900 transition-all"
                autofocus
              />
            </form>
          </div>
