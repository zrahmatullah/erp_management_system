<template>
  <div class="h-[calc(100vh-5.5rem)] flex flex-col space-y-4">
    <!-- Top Header Bar -->
    <div class="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm dark:shadow-lg shrink-0 border border-slate-200/80 dark:border-slate-800 transition-colors">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400">Antrian Pesanan Dapur & Barista Real-time</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <div class="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">
          <button
            ...
            :class="stationFilter === st ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'"
          >
        ...
        <button
          ...
          class="p-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white rounded-xl border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
        ...
        <!-- Real-time Clock -->
        <div class="flex items-center gap-2 font-mono text-lg font-black bg-slate-100 dark:bg-slate-800 px-4 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-emerald-600 dark:text-emerald-400 shadow-inner">
    ...
    <div class="bg-white/90 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl p-2 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between gap-2 overflow-x-auto shrink-0 shadow-xs">
      <div class="flex items-center gap-2">
        ...
        >
          ...
          <span
            ...
            :class="activeTypeFilter === tab.id ? 'bg-white text-blue-700' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300'"
          >
      ...
      <!-- Quick Legend Badge -->
      <div class="hidden lg:flex items-center gap-3 pr-3 text-[11px] font-bold text-slate-500 dark:text-slate-400">
    ...
    <div class="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 min-h-0 overflow-hidden">
      ...
      <div class="flex flex-col h-full bg-slate-100/70 dark:bg-slate-900/70 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3">
          <div
            ...
            :key="t.id"
            class="bg-white dark:bg-slate-900 p-4 rounded-xl border border-amber-500/40 shadow-xs dark:shadow-md space-y-3 hover:border-amber-500 transition-all"
          >
            ...
            <div class="flex items-start justify-between gap-2">
              <div>
                ...
                <div class="text-[11px] font-bold text-slate-700 dark:text-slate-300 mt-1 flex items-center gap-1.5">
                  ...
                  <span class="text-slate-500 dark:text-slate-400 font-normal truncate max-w-[100px]">{{ t.customerName }}</span>
                </div>
              </div>
            ...
            <!-- Item Detail Box -->
            <div class="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200/80 dark:border-slate-800 space-y-1.5">
              <div class="flex items-start justify-between gap-2">
                <div class="flex items-center gap-2">
                  <span class="w-6 h-6 rounded-md bg-amber-500/20 text-amber-600 dark:text-amber-400 font-black text-xs flex items-center justify-center">
                    {{ t.quantity }}x
                  </span>
                  <span class="font-bold text-slate-900 dark:text-white text-xs">{{ t.productName }}</span>
                </div>
                <span class="px-2 py-0.5 rounded text-[9px] font-bold uppercase bg-slate-200/80 dark:bg-slate-800 text-slate-700 dark:text-slate-400">
                  {{ t.station }}
                </span>
              </div>

              <div v-if="t.notes" class="text-[11px] text-amber-700 dark:text-amber-300 italic bg-amber-50 dark:bg-amber-950/30 p-1.5 rounded border border-amber-200 dark:border-amber-900/50">
                Catatan: "{{ t.notes }}"
              </div>
            </div>
          ...
          <div v-if="pendingTickets.length === 0" class="h-40 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 text-xs space-y-1">
            <CheckCircle2 class="w-8 h-8 text-slate-300 dark:text-slate-700" />
      ...
      <div class="flex flex-col h-full bg-slate-100/70 dark:bg-slate-900/70 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3">
          <div
            ...
            :key="t.id"
            class="bg-white dark:bg-slate-900 p-4 rounded-xl border border-blue-500/50 shadow-xs dark:shadow-md space-y-3 hover:border-blue-400 transition-all"
          >
            <!-- Header -->
            <div class="flex items-start justify-between gap-2">
              <div>
            ...
            <div class="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200/80 dark:border-slate-800 space-y-1.5">
              <div class="flex items-start justify-between gap-2">
                <div class="flex items-center gap-2">
                  <span class="w-6 h-6 rounded-md bg-blue-500/20 text-blue-600 dark:text-blue-400 font-black text-xs flex items-center justify-center">
                    {{ t.quantity }}x
                  </span>
                  <span class="font-bold text-slate-900 dark:text-white text-xs">{{ t.productName }}</span>
                </div>
          ...
          <div v-if="inProgressTickets.length === 0" class="h-40 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 text-xs space-y-1">
            <Flame class="w-8 h-8 text-slate-300 dark:text-slate-700" />
      ...
      <div class="flex flex-col h-full bg-slate-100/70 dark:bg-slate-900/70 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3">
          <div
            ...
            :key="t.id"
            class="bg-white dark:bg-slate-900 p-4 rounded-xl border border-emerald-500/50 shadow-xs dark:shadow-md space-y-3 hover:border-emerald-400 transition-all"
          >
            <!-- Header -->
            <div class="flex items-start justify-between gap-2">
              <div>
              ...
              <span class="px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-extrabold flex items-center gap-1">
            ...
            <div class="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200/80 dark:border-slate-800 space-y-1.5">
              <div class="flex items-start justify-between gap-2">
                <div class="flex items-center gap-2">
                  <span class="w-6 h-6 rounded-md bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-black text-xs flex items-center justify-center">
                    {{ t.quantity }}x
                  </span>
                  <span class="font-bold text-slate-900 dark:text-white text-xs">{{ t.productName }}</span>
                </div>
            ...
            <button
              ...
              class="w-full py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/30 font-black text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
            >
          ...
          <div v-if="readyTickets.length === 0" class="h-40 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 text-xs space-y-1">
            <BellRing class="w-8 h-8 text-slate-300 dark:text-slate-700" />
      ...
      <!-- 4. Disajikan (Served) -->
      <div class="flex flex-col h-full bg-slate-100/70 dark:bg-slate-900/70 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden">
        <div class="px-4 py-3 bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-slate-300 font-black text-xs uppercase tracking-wider flex items-center justify-between shrink-0 shadow-sm">
          <div class="flex items-center gap-2">
            <CheckCircle2 class="w-4 h-4 text-slate-500 dark:text-slate-400" />
            <span>Selesai (Served)</span>
          </div>
          <span class="px-2 py-0.5 rounded-full bg-black/10 dark:bg-black/20 text-xs font-black">{{ servedTickets.length }}</span>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3">
          <div
            ...
            :key="t.id"
            class="bg-white/80 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 space-y-2.5 opacity-80"
          >
            <div class="flex items-center justify-between">
              <span class="font-black text-xs text-slate-800 dark:text-slate-300">#{{ t.queueNumber }} • {{ t.productName }}</span>
              <span class="text-[10px] text-slate-400 dark:text-slate-500 font-mono">{{ t.time }}</span>
            </div>
          ...
          <div v-if="servedTickets.length === 0" class="h-40 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 text-xs space-y-1">
...
const getTimerClass = (minutes: number) => {
  if (minutes < 10) return 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/40'
  if (minutes < 20) return 'bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-400 border border-amber-200 dark:border-amber-800/40'
  return 'bg-red-100 text-red-800 dark:bg-red-950/80 dark:text-red-400 border border-red-200 dark:border-red-800/60 animate-pulse'
}
